...
## IMPLEMENTATION RECORD — WS-COPY-VOICE (2026-09-05)

Applied pairs 1, 2, 3, 4, 5, 7, 8, 9, 12, 13, 14 (11 of 14) via string-keyed edits at HEAD `b6a87e8` (worktree carried live sibling in-flight edits from the parallel round-2 waves — motion / mobile / a11y — all anchors re-grepped at edit time, no line-keyed edits). render.test.js and wow.test.js: ZERO-DIFF (no pin collided with any applied before-string, as predicted). Suite gate: fail 0.

**SKIPS (3, with tree evidence re-verified 2026-09-05):**
- **Pair 6 — SKIP (contract-pinned sibling fix).** The static flow-deposit first paint already reads deployed truth: `site/index.html:265` flow-deposit-state = `open — approve the vault, then deposit` (pinned exactly-once by wow.test.js: `the flow deposit node ships the deployed register statically, exactly once`). The pending variant's single home is CONTRACT-PINNED: `site/js/main.js:30` `FLOW_DEPOSIT_SUB.pending` (`schematic — deposits activate when the vault deploys`), and wow.test.js asserts index.html ABSENCE + main.js PRESENCE of that sentence (`the pending sentence never ships statically (Branch B)` / `the pending sentence lives in main.js's FLOW_DEPOSIT_SUB writer`). Deleting the main.js remnant fails the suite. Skip; never "clean up" the remnant.
- **Pair 10 — SKIP (WS-OG-PERF frozen-copy re-lock).** The og:description (`site/index.html:8`) is re-locked frozen copy: `site-tests/theme.test.js:255` pins the contiguous fragment `No audit. Every number is read by your browser straight from public chain nodes.` at count === 1, and the pin comment states the short variant "stays pinned". Pair 10's after inserts `No dashboard. ` between `No audit.` and `Every number`, which breaks that pin — and theme.test.js is not an edit site. (The mapper's own "Tests: none found" note for pair 10 was stale: it swept full before-strings, not the pinned sub-fragment.)
- **Pair 11 — SKIP (superseded by WS-PRODUCT-GAPS P5).** Both coverage cells already carry the strictly better live-read string `…verify it yourself with any RPC client.` (verified ×2 in site/index.html at implementation time, re-pinned in agent-first.test.js as `the noscript coverage truth is exactly the two static seam cells`). Applying pair 11 as written would regress a sibling's landed fix.

**PAIR 7 — deliberately out-of-scope lowercase sites (the rename was scoped to 4 label sites, one string each):** the flow-node sub `site/index.html:270` (`protocol cut — connecting to public RPC…`), the main.js flow writer `site/js/main.js:693-694` (`protocol cut … (live slot0 …)` / `protocol cut — unavailable (RPC)`), and the code comment `site/js/main.js:346` (`// 3. Protocol cut — decoded LIVE from slot0's feeProtocol word`) all legitimately keep the lowercase register and were NOT touched. Implementation note: the two main.js label sites use double-quoted JS strings (`"The pool owner's cut (live)"` / `"The pool owner's cut"`) so the file bytes carry the plain apostrophe (no `\'` escape) — double-quoted strings have precedent in main.js (e.g. the doc-tab selector string) and the plain bytes are what the verify greps count (live ×3 across both files, degraded ×1 in main.js, counted via the longer live form to defeat the prefix overlap).

**PAIR 3 — dynamic-button note (the claim is verifiably true):** main.js's wallet-picker buttons are created at `site/js/main.js:1193` (`el('button', 'btn picker-btn', …)`) and only OPEN the wallet-selection picker — they move no funds. The four static fund-movers in section#deposit (`#btn-connect` index.html:381, `#btn-deposit` :344, plus the redeem/withdraw pair) remain the only buttons on the page that can move funds, so the quiet line's scope statement is accurate.

**PAIR 4 — out-of-scope kept site:** `site/js/config.js:31` keeps the lowercase protocol name `Wellstreet — open-source yield vaults for tokenized stocks` — never edited (out of pair-4 scope by design; it feeds surfaces outside this goal's scope).

**CSS:** the two mandated rules landed adjacent to the `#agents .quiet` house rule (top-level, mirroring the `#invariants`/`#agents` trailing-comment form): `#deposit .quiet { color: var(--ink-soft); }` and `#docs .quiet { color: var(--ink-soft); }` — total `quiet { color: var(--ink-soft); }` lines now 5 (was 3).

**Idempotency note:** all 11 before-strings were grep-verified present (exact expected counts) and all 11 after-strings absent immediately before the edits — none of the pairs had been applied by a sibling first.
